...
import ClassroomTransformation from "@/components/ClassroomTransformation";
import Footer from "@/components/Footer";
import Link from "next/link";
import { Sparkles } from "lucide-react";

export const metadata = {
  title: "Our Projects | Face of Change",
  description: "Explore our solar grids energy projects in Odisha, our healthcare diagnostics camps in Delhi, and classroom transformations.",
};
        ...
        <div className="bg-[#FAF9F5] text-navy pt-36 pb-20 relative overflow-hidden border-b border-navy/5">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 space-y-4">
            ...
            <nav className="flex space-x-2 text-xs font-semibold uppercase tracking-wider text-navy/40">
              ...
              <span>/</span>
              <span className="text-navy">Our Projects</span>
            </nav>
            ...
            <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold leading-none tracking-wide text-navy">
              Our <span className="text-forest">Projects</span>
            </h1>
            <p className="text-navy/60 text-sm sm:text-base max-w-2xl leading-relaxed font-sans">
              Discover the projects we run to build sustainable ecosystems, power villages, transform classrooms, and deliver healthcare.
            </p>
          </div>
        </div>

        {/* Content sections */}
        <FeaturedProjects isOverview={false} />
        <ClassroomTransformation />
      </main>
      <Footer />
    </>
  );
